...
rChain <- R6::R6Class(
  ...
  public = list(
    ...
    initialize = function() {
      private$keypair <- ec_keygen("P-521")
      self$createBlock()
    },
    ...
    addItem = function(data = NULL) {
      ...
      ck <- base64_encode(signature_create(charToRaw(ck), sha512, private$keypair))
    ...
    createBlock = function() {
      ...
      if (is.null(private$items_pool)) {
        ...
        ck <- signature_create(charToRaw(ck_str), sha512, private$keypair)
    ...
    validateBlock = function(num) {
      ...
      if (body_val$Result) {
        ...
        chk_result <- try(signature_verify(charToRaw(ck_str),
                                           base64_decode(private$blocks[[num]]$Header$Check),
                                           sha512,
                                           private$keypair$pubkey),
                          silent = TRUE)
        if (class(chk_result) == "try-error"){
